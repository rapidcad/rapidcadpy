...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Reverse engineer IPT files to Python code."
    )
    parser.add_argument("--file", help="Path to a single IPT file to process")
    parser.add_argument(
        "--output", help="Path to the output Python file (used with --file)"
    )
    parser.add_argument("--dir", help="Directory containing IPT files to process")
    parser.add_argument(
        "--outdir", help="Output directory for generated Python files (used with --dir)"
    )
    parser.add_argument(
        "--recursive", action="store_true", help="Recursively search for IPT files"
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
    ...
    if args.file and args.output:
        ...
        if not os.path.exists(args.file):
            ...
            reverse_engineer_ipt_file(
                abs_file_path, args.output, overwrite=args.overwrite
            )
        ...
        if not os.path.exists(args.dir):
            ...
            reverse_engineer_directory(
                abs_dir_path, args.outdir, args.recursive, overwrite=args.overwrite
            )
        ...
        reverse_engineer_directory(
            input_dir, output_dir, recursive=True, overwrite=args.overwrite
        )
