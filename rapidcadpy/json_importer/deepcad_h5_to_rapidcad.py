...
def convert_h5_to_cadquery(
    ...
    for i, sketch in enumerate(sketches):
        ...
        if type == EXTENT_TYPE.index("OneSideFeatureExtentType"):
            if use_fixed_decimal is True:
                python_rapidcad.append(
                    f"wp_sketch{i}.extrude({dir1:.{truncate}f}, '{op_str}')\n"
                )
            else:
            ...
            if use_fixed_decimal is True:
                python_rapidcad.append(
                    f"wp_sketch{i}.extrude({pos:.{truncate}f}, '{op_str}')\n"
                )
            else:
            ...
            if use_fixed_decimal is True:
                python_rapidcad.append(
                    f"wp_sketch{i}.extrude({neg:.{truncate}f}, '{op_str}')\n"
                )
            else:
            ...
            if dir2 == 0:
                ...
                if use_fixed_decimal is True:
                    ...
                    python_rapidcad.append(
                        f"wp_sketch{i}.extrude({dir1}, '{op_str}')\n"
                    )
            else:
                if (dir1 > 0) and (dir2 > 0):
                    dir_with_normal = dir1
                ...
                if use_fixed_decimal is True:
                    python_rapidcad.append(
                        f"wp_sketch{i}.extrude({dir_with_normal:.{truncate}f}, '{op_str}')\n"
                    )
                else:
                    python_rapidcad.append(
                        f"wp_sketch{i}.extrude({dir_with_normal}, '{op_str}')\n"
                    )
                # Repeat sketch geometry for the opposite side
                python_rapidcad.append(wp_decl)
                python_rapidcad.append(loop_chunk)
                if use_fixed_decimal is True:
                    python_rapidcad.append(
                        f"wp_sketch{i}.extrude({dir_against_normal:.{truncate}f}, '{op_str}')\n"
                    )
                else:
                    python_rapidcad.append(
                        f"wp_sketch{i}.extrude({dir_against_normal}, '{op_str}')\n"
                    )
