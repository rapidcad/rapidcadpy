...
def convert_h5_to_cadquery(vecs, save_python_dir, save_mesh_path, use_fixed_decimal, truncate):
    """
    ...
    def rapidcad_line(x, y, curr_x, curr_y, loop_list, unquantize, extrude_scale):
        if (x == curr_x) and (y == curr_y): # handles case where there are duplicate points or first point is zero and it also closes at zero, need to handle case of first point and last point are same but not zero
            ...
            if unquantize:
                ...
                y = (y + translate)*scale
            if len(loop_list) == 0:
                return f".move_to({x:.{truncate}f}, {y:.{truncate}f})" if use_fixed_decimal else f".move_to({x}, {y})"
            else:
                return f".line_to({x:.{truncate}f}, {y:.{truncate}f})" if use_fixed_decimal else f".line_to({x}, {y})"
        
    def rapidcad_arc(x, y, curr_x, curr_y, sweep, dir_flag, loop_list, unquantize, extrude_scale):
    ...
    def rapidcad_circle(x, y, r, loop_list, unquantize, extrude_scale):
        if unquantize:
            ...
            r = r*scale
        if len(loop_list) == 0:
            return f".move_to({x:.{truncate}f}, {y:.{truncate}f}).circle({r:.{truncate}f})" if use_fixed_decimal else f".move_to({x}, {y}).circle({r})"
        else:
    ...
    def rapidcad_close_loop(loop_operations, loop_num, sketch_num):
        if len(loop_operations) > 1:
            ...
            if "circle" in loop_operations[0]:
                ...
                raise NotImplementedError(f"Loop content not supported: {loop_operations}")
    ...
    def cadquery_extrude(loops_joined, op, type, dir1, dir2, sketch_num, normal, repeat_sketch, repeat_loops):
        ...
        if sketch_num == 0: #TODO: futher investigate case 80443. DeepCAD doesn't actually join the bodies? If the bodies are joined, shouldn't lines be removed from the pieces?
            if (op == EXTRUDE_OPERATIONS.index("NewBodyFeatureOperation")) or (op == EXTRUDE_OPERATIONS.index("JoinFeatureOperation")) or (op == EXTRUDE_OPERATIONS.index("CutFeatureOperation")) or (op == EXTRUDE_OPERATIONS.index("IntersectFeatureOperation")):
                if type == EXTENT_TYPE.index("OneSideFeatureExtentType"):
                    ...
                    extrude_command_with_normal = f".extrude({dir_with_normal})\n" if not use_fixed_decimal else f".extrude({dir_with_normal:.{truncate}f})\n"
                    extrude_command_against_normal = f".extrude({dir_against_normal})\n" if not use_fixed_decimal else f".extrude({dir_against_normal:.{truncate}f})\n"
        ...
        # handle non first body cases
            ...
            if (op == EXTRUDE_OPERATIONS.index("NewBodyFeatureOperation")) or (op == EXTRUDE_OPERATIONS.index("JoinFeatureOperation")):
                if type == EXTENT_TYPE.index("OneSideFeatureExtentType"):
                    extrude_command = f".extrude({dir1})\n" if not use_fixed_decimal else f".extrude({dir1:.{truncate}f})\n"
                    return f"solid{sketch_num}=wp_sketch{sketch_num}" + loops_joined + extrude_command + f"solid=solid.union(solid{sketch_num})\n"
                elif type == EXTENT_TYPE.index("SymmetricFeatureExtentType"):
                    extrude_command = f".extrude({dir1}, both=True)\n" if not use_fixed_decimal else f".extrude({dir1:.{truncate}f}, both=True)\n"
                    return f"solid{sketch_num}=wp_sketch{sketch_num}" + loops_joined + extrude_command + f"solid=solid.union(solid{sketch_num})\n"
                else: # two sided extent
        ...
        for loop in loops:
            ...
            for s_ind, sketch_op in enumerate(loop):
                if sketch_op[0] == LINE:
                    point_x = sketch_op[1]
                    point_y = sketch_op[2]
                    cmd = rapidcad_line(point_x, point_y, curr_x, curr_y, loop_ops, UNQUANTIZE, scale)
                    curr_x = point_x
                    curr_y = point_y
                    loop_ops.append(cmd)
                    ...
                    cmd = rapidcad_arc(end_x, end_y, curr_x, curr_y, sweep_angle, directional_flag, loop_ops, UNQUANTIZE, scale)
                    curr_x = end_x
                    curr_y = end_y
                    loop_ops.append(cmd)
                    ...
                    cmd = rapidcad_circle(center_x, center_y, circle_radius, loop_ops, UNQUANTIZE, scale)
                    loop_ops.append(cmd)
                else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="h5 to rapidcadpy (Inventor) conversion")
...
#file_path_ques = str(input("What file path would you like to save the generated python files to? (default: deepcad_derived/data/cad_vec/cadquery): "))
    ...
    for sub_dir in sub_dirs:
        ...
        for i, h5_vec_path in enumerate(h5_files):
            ...
            try:
                convert_h5_to_cadquery(h5_vec, python_file_save_path, stl_file_save_path, args.use_fixed_decimal, args.decimal_points)
                ...
                if generate_stls:
                    try:
                        ...
                        if ("/0002/00024147.h5" not in h5_vec_path) and ("/0005/00057125.h5" not in h5_vec_path) and ("/0012/00126522.h5" not in h5_vec_path) and ("/0014/00140564.h5" not in h5_vec_path): #handle these weird hanging cases
                            ...
                            if step_checker(python_file_save_path): # check if the generated python file produces a valid STL  file
                                gen_file += 1
