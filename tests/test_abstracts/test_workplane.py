...
class TestFluentAPIIntegration:
    ...
    def test_cad_object_has_to_inventor_method(self):
        ...
        solid = wp.circle(1).close().extrude(1)
    ...
    def test_step_export_works(self, tmp_path):
        ...
        wp = Workplane(Plane(Vector(0, 0, 0), Vector(1, 0, 0), Vector(0, 1, 0)))
        solid = wp.circle(0.5).close().extrude(1)
    ...
    def test_multiple_extrusions(self):
        ...
        # Create first solid
        wp1 = Workplane(plane)
        solid1 = wp1.moveTo(0, 0).circle(1).close().extrude(1)

        # Create second solid
        wp2 = Workplane(plane)
        solid2 = wp2.moveTo(5, 5).rect(2, 2).close().extrude(0.5)
...
class TestErrorHandling:
    ...
    def test_negative_extrude_distance(self):
        ...
        wp = Workplane(Plane(Vector(0, 0, 0), Vector(1, 0, 0), Vector(0, 1, 0)))
        solid = wp.circle(1).close().extrude(-1.0)
