...
class TestShapeVolume:
    ...
    def test_box_volume(self, app):
        ...
        assert (
            abs(calculated_volume - expected_volume) < 0.01
    ...
    def test_cylinder_volume(self, app):
        ...
        assert (
            abs(calculated_volume - expected_volume) < 0.1
    ...
    def test_complex_shape_volume(self, app):
        ...
        assert (
            result_volume > expected_min
        ), "Result volume should account for partial hole"
    ...
    def test_union_volume(self, app):
        ...
        assert result_volume < (
            volume1 + volume2
        ), "Union should be less than sum due to overlap"
        assert result_volume > max(
            volume1, volume2
        ), "Union should be larger than either box"
    ...
    def test_union_multiple_shapes(self, app):
        ...
        assert (
            result_volume > base_volume
        ), "Union with multiple shapes should increase volume"
        assert result_volume > 0, "Result volume should be positive"
...
class TestPipe:
    ...
    def test_truss_pipe(self):
        ...
        # create bridge body
        wp.move_to(0, 0).line_to(20, 0).line_to(20, -height).line_to(
            0, -height
        ).close().extrude(width)

        # create left truss
        wp.move_to(0, 0).line_to(5, 5).line_to(10, 0).line_to(15, 5).line_to(
            20, 0
        ).close().pipe(diameter=1)

        # create right truss
        wp_right = app.work_plane("XZ", offset=width)
        wp_right.move_to(0, 0).line_to(5, 5).line_to(10, 0).line_to(15, 5).line_to(
            20, 0
        ).close().pipe(diameter=1)
        app.show_3d()
